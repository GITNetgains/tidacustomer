...
@objc class AppDelegate: FlutterAppDelegate, PayWithEasebuzzCallback {
  ...
  func initiatePaymentAction(call: FlutterMethodCall) {
    if let orderDetails = call.arguments as? [String: String] {
      ...
      // handle error
      let dict = self.setErrorResponseDictError(
        "Empty error", errorMessage: "Invalid validation", result: "Invalid request")
      self.payResult(dict)
    }
  }
  ...
  func PEBCallback(data: [String: AnyObject]) {
    if data.count > 0 {
      ...
      let dict = self.setErrorResponseDictError(
        "Empty error", errorMessage: "Empty payment response", result: "payment_failed")
      self.payResult(dict)
    }
  }
